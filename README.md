...
每台机器运行同一个程序。其中一台作为 **Primary（主机，连着真实的鼠标键盘）**，其余作为 **Secondary（从机，接收输入）**。界面里可以像操作系统显示器设置那样**拖动每台电脑的屏幕位置**——把屏幕边缘对齐，鼠标就能平滑地“跨”到另一台电脑上。副机色块可以拖到主屏的**上 / 下 / 左 / 右任意一边**（靠近时会自动吸附对齐），四个方向都能穿越。
...
- 📋 剪贴板双向同步（文本 + **文件**，基于 [arboard](https://github.com/1Password/arboard)，带回环抑制）
- 🖥️ 可拖拽的屏幕布局（基于 [egui](https://github.com/emilk/egui)），所见即所得；**四向穿越**（上 / 下 / 左 / 右）
- 📐 本机屏幕位置由系统决定（不可拖动），副机色块靠近本机屏幕时自动**吸附对齐**，并画出可穿越的共享边
- 🌍 **中文 / English 界面一键切换**（左侧边栏底部，选择会保存；含启动错误提示）
- 🎛️ 三段式界面：**连接**（角色与网络）/ **屏幕布局**（可视化拖拽）/ **状态**（连接统计与真实事件流）
...
1. 启动 `mouseshare`，在「连接」页把角色选为 **主机（Primary）**。
2. 点击 **探测本机局域网 IP** 获取本机局域网地址（如 `192.168.1.20:49152`）。
3. 在「屏幕布局」页把这台机器的屏幕放好（默认已在原点）。

### 2. 在其它机器上当 Secondary（从机）

1. 启动 `mouseshare`，在「连接」页把角色选为 **副机（Secondary）**。
2. 填入 Primary 的地址（`192.168.1.20:49152`）。可以在 Primary 界面点 **复制** 一键复制，再粘过来。
3. 连上后，这块屏幕会**自动出现**在 Primary 的布局里（客户端数量无上限）。把它拖到 Primary 屏幕旁边即可，无需手动添加。
   - Primary 的「连接」页 → **本地屏幕** 里也可以用 **复制** / **删除** 管理屏幕：复制会克隆当前屏幕（名称自动加 `-copyN`、并右移避免重叠），方便快速搭好多机布局。
4. **重要**：每台机器的屏幕 `name` 必须和那台机器配置里的 `name` 一致（默认是主机名）。**不需要手工编辑布局文件**——每台机器连上后会把本机屏幕（含多显示器）自动上报给主机，主机负责排布并把布局同步给所有机器。

### 3. 使用

把真实鼠标移到 Primary 屏幕边缘、对准邻居屏幕的方向，继续推——光标会“跨”到邻居屏幕上，键盘和剪贴板也随之切换过去。往回推则切回 Primary。

**文件复制**：在任意一台机器上 `Cmd/Ctrl+C` 复制文件（或文件夹），会在其它机器上自动接收；切到那台机器后直接 `Cmd/Ctrl+V` 即可粘贴。单个/单次复制上限 512 MB，接收到的文件放在系统临时目录下（`.../mouseshare/clipboard/`）。
...
- 剪贴板同步**文本与文件**；图片尚未支持。文件复制在 Linux 上不可用（X11 剪贴板格式各异）。
